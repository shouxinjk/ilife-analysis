...
import com.ilife.analyzer.spout.person.EvaluateDimensionSpout;
 ...
 * 3，将得分更新到evaluate：update evaluate set score=?,modifiedOn=now(),status="ready" where userKey=? and evalute=? and type=?
 *
 */
public class EvaluateByDimension extends AbstractTopology {
	    ...
	    public FlinkTopology getTopology() {
            ...
            String sql = "select sum(weight*score) as score, ? as userKey, ? as evaluation,? as type from user_evaluation where userKey=? and parent=?";
            ...
            String[] output_fields = {"score","userKey","evaluation","type"};
            ...
            JdbcInsertBolt jdbcUpdateEvaluateBolt = new JdbcInsertBolt(analyzeConnectionProvider, updateMapper)
                    .withInsertQuery("update user_evaluation set score=?,modifiedOn=now(),status='ready' where userKey=? and evaluation=? and type=?");

            String nodeSpout = "evaluate_user_dimension_spout";
            String nodeCalcScore = "evaluate_user_dimension_calc_score";
            String nodeUpdateScore = "evaluate_user_dimension_update_score";
