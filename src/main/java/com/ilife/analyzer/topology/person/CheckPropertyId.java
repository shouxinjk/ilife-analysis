...
import com.ilife.analyzer.spout.person.PropertyIdSpout;
import com.ilife.analyzer.topology.AbstractTopology;

/**
 * @author alexchew
 * 根据property得到属性定义的propertyId。用于准备进行维度分析
 * 1，从分析库property内读取propertyId为null的记录，返回property
 * 2，根据proptery从业务库内读取propertyId
...
public class CheckPropertyId extends AbstractTopology {
	    ...
	    public FlinkTopology getTopology() {
	    		...
	    		//2，读取对应的标注value记录
            String sql = "select id as propertyId,property as propertyName from mod_user_measure where property=?";
            List<Column> queryParamColumns = Lists.newArrayList(
            		new Column("propertyName", Types.VARCHAR));
            String[] output_fields = {"propertyId","propertyName"};
            ...
            JdbcInsertBolt jdbcUpdateBolt = new JdbcInsertBolt(analyzeConnectionProvider, updateMapper)
                    .withInsertQuery("update user_property set propertyId=? where property=?");

            //装配topology
            String spout = "check_user_propertyId_spout";
            String findScoreBolt = "check_user_propertyId_find_id";
            String updateProperyBolt = "check_user_propertyId_update_id";
