...
import com.ilife.analyzer.spout.person.PropertySpout;
...
public class Normalize extends AbstractTopology {
	    ...
	    public FlinkTopology getTopology() {
	    		...
	    		//2，读取对应的标注value记录
            String sql = "select score,rank,property,value from user_value where property=? and value=? and status='ready'";
            List<Column> queryParamColumns = Lists.newArrayList(
            		new Column("property", Types.VARCHAR),
            		new Column("value", Types.VARCHAR));
            String[] output_fields = {"score","rank","property","value"};
            ...
            JdbcInsertBolt jdbcUpdateBolt = new JdbcInsertBolt(analyzeConnectionProvider, updateMapper)
                    .withInsertQuery("update user_property set score=?,rank=?,modifiedOn=now(),status='ready' where property=? and value=?");

            //装配topology
            String spout = "normalize_user_spout";
            String findScoreBolt = "normalize_user_find_score";
            String updateProperyBolt = "normalize_user_update_property_score";
