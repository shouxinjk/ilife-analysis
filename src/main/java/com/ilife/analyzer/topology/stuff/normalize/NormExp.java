...
/**
 * 对指定categoryId、propertyId的数值进行指数归一化。
 ...
 *         +---3.3
 */
public class NormExp extends AbstractTopology {

	    public static void main(String[] args) throws Exception {
	        new NormExp().execute(args);
	    }

	    @Override
	    public StormTopology getTopology() {
	    	//1，获取待补充marked_value的数值记录
	    	NormSpout propertySpout = new NormSpout(businessConnectionProvider,"exp");
            ...
            JdbcInsertBolt normUpdateBolt = new JdbcInsertBolt(businessConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update ope_performance set normalized_value=exp(marked_value)/?,update_date=now() where category_id=? and measure_id=?");
            //更新value
            JdbcInsertBolt normUpdateBoltValue = new JdbcInsertBolt(analyzeConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update `value` set score=exp(marked_value)/?,modifiedOn=now() where categoryId=? and propertyId=?");
            //更新property
            JdbcInsertBolt normUpdateBoltProp = new JdbcInsertBolt(analyzeConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update property set score=exp(marked_value)/?,modifiedOn=now() where categoryId=? and propertyId=?");
            
            //装配topology
	        TopologyBuilder builder = new TopologyBuilder();
	        builder.setSpout("norm-exp-spout", propertySpout, 1);
	        builder.setBolt("norm-exp-find", jdbcFindScoreBolt, 1).shuffleGrouping("norm-exp-spout");
	        builder.setBolt("norm-exp-set", normUpdateBolt, 1).shuffleGrouping("norm-exp-find");
	        builder.setBolt("norm-exp-set-value", normUpdateBoltValue, 1).shuffleGrouping("norm-exp-find");
	        builder.setBolt("norm-exp-set-prop", normUpdateBoltProp, 1).shuffleGrouping("norm-exp-find");
