...
public class EvaluateByDimension extends AbstractTopology {
	    ...
	    public StormTopology getTopology() {
	    		...
	    		MeasureDimensionSpout nodes = new MeasureDimensionSpout(analyzeConnectionProvider);
	    		
	    		//2，JdbcLookupBolt：查询并汇总叶子节点关联属性的加权得分，对应于特定itemKey，并从属于当前dimension
	    		//注意：当前默认采用加权汇总算法，如果采用单独算法，需要定义相应的Bolt
            String sql = "select sum(weight*score) as score, ? as itemKey, ? as evaluate,? as type from evaluate where itemKey=? and parent=?";
            List<Column> queryParamColumns = Lists.newArrayList(
            		new Column("itemKey", Types.VARCHAR),
            		new Column("evaluate", Types.VARCHAR),
            		new Column("type", Types.VARCHAR),
            		new Column("itemKey2", Types.VARCHAR),
            		new Column("evaluate2", Types.VARCHAR));
            String[] output_fields = {"score","itemKey","evaluate","type"};
            ...
            JdbcInsertBolt jdbcUpdateEvaluateBolt = new JdbcInsertBolt(analyzeConnectionProvider, updateMapper)
                    .withInsertQuery("update measure set score=?,modifiedOn=now(),status='ready' where itemKey=? and evaluate=? and type=?");
