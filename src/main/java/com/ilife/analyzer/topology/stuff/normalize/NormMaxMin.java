...
/**
 * 对指定categoryId、propertyId的数值进行max-min归一化。
 ...
 * 
 * 步骤：
 * 1，获取max-min归一化的属性列表，同时返回categoryId、propertyId
 ...
 *         +---3.3
 */
public class NormMaxMin extends AbstractTopology {

	    public static void main(String[] args) throws Exception {
	        new NormMaxMin().execute(args);
	    }

	    @Override
	    public StormTopology getTopology() {
	    	//1，获取待补充marked_value的数值记录
	    	NormSpout propertySpout = new NormSpout(businessConnectionProvider,"max-min");
...
//            		,new Column("max", Types.DOUBLE)
            ...
            JdbcInsertBolt normUpdateBolt = new JdbcInsertBolt(businessConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update ope_performance set normalized_value=1-((marked_value-?)/?),update_date=now() where category_id=? and measure_id=?");
            //更新value
            JdbcInsertBolt normUpdateBoltValue = new JdbcInsertBolt(analyzeConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update `value` set score=1-((marked_value-?)/?),modifiedOn=now() where categoryId=? and propertyId=?");
            //更新property
            JdbcInsertBolt normUpdateBoltProp = new JdbcInsertBolt(analyzeConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update property set score=1-((marked_value-?)/?),modifiedOn=now() where categoryId=? and propertyId=?");
            
            //装配topology
	        TopologyBuilder builder = new TopologyBuilder();
	        builder.setSpout("norm-max-min-spout", propertySpout, 1);
	        builder.setBolt("norm-max-min-find", jdbcFindScoreBolt, 1).shuffleGrouping("norm-max-min-spout");
	        builder.setBolt("norm-max-min-set", normUpdateBolt, 1).shuffleGrouping("norm-max-min-find");
	        builder.setBolt("norm-max-min-set-value", normUpdateBoltValue, 1).shuffleGrouping("norm-max-min-find");
	        builder.setBolt("norm-max-min-set-prop", normUpdateBoltProp, 1).shuffleGrouping("norm-max-min-find");
