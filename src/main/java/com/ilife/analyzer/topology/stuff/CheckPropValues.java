...
import com.ilife.analyzer.bolt.stuff.InsertPropValueBolt;
...
public class CheckPropValues extends AbstractTopology {
	    ...
	    public StormTopology getTopology() {
            ...
            //注意：采用insert on duplicate key方式，如果重复则只更新时间戳。唯一性标记为categoryId、propertyId、value值
    		//需要根据property的标注类型，分别将数值写入ope_performance、对应的字典表 
    		InsertPropValueBolt insertPropValueBolt = new InsertPropValueBolt(businessConnectionProvider,analyzeConnectionProvider); 
            
            //装配topology
	        TopologyBuilder builder = new TopologyBuilder();
	        builder.setSpout("check_prop_values", propValesSpout, 1);
	        builder.setBolt("sync_performance_value", insertPropValueBolt, 1).shuffleGrouping("check_prop_values");
