 ...
 * 
 * 算法：修正所有标注值到[1,*）
 * 1-log(1+x)/log(1+max(x))
 * 
 * 步骤：
 * 1，获取logx-reverse归一化的属性列表，同时返回categoryId、propertyId
 ...
 *         +---3.3
 */
public class NormLogxReverse extends AbstractTopology {

	    public static void main(String[] args) throws Exception {
	        new NormLogxReverse().execute(args);
	    }

	    @Override
	    public StormTopology getTopology() {
	    	//1，获取待补充marked_value的数值记录
	    	NormSpout propertySpout = new NormSpout(businessConnectionProvider,"logx-reverse");
            ...
            JdbcInsertBolt normUpdateBolt = new JdbcInsertBolt(businessConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update ope_performance set normalized_value=1-log(1+marked_value)/?,update_date=now() where category_id=? and measure_id=?");
            //更新value
            JdbcInsertBolt normUpdateBoltValue = new JdbcInsertBolt(analyzeConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update `value` set score=1-log(1+marked_value)/?,modifiedOn=now() where categoryId=? and propertyId=?");
            //更新property
            JdbcInsertBolt normUpdateBoltProp = new JdbcInsertBolt(analyzeConnectionProvider, normUpdateMapper)
                    .withInsertQuery("update property set score=1-log(1+marked_value)/?,modifiedOn=now() where categoryId=? and propertyId=?");
            
            //装配topology
	        TopologyBuilder builder = new TopologyBuilder();
	        builder.setSpout("norm-logx-reverse-spout", propertySpout, 1);
	        builder.setBolt("norm-logx-reverse-find", jdbcFindScoreBolt, 1).shuffleGrouping("norm-logx-reverse-spout");
	        builder.setBolt("norm-logx-reverse-set", normUpdateBolt, 1).shuffleGrouping("norm-logx-reverse-find");
	        builder.setBolt("norm-logx-reverse-set-value", normUpdateBoltValue, 1).shuffleGrouping("norm-logx-reverse-find");
	        builder.setBolt("norm-logx-reverse-set-prop", normUpdateBoltProp, 1).shuffleGrouping("norm-logx-reverse-find");
