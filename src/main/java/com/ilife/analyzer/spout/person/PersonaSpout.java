 ...
 *
 */
public class PersonaSpout extends BaseRichSpout implements IRichSpout {
    ...
    private static final Logger logger = Logger.getLogger(PersonaSpout.class);
    
    public PersonaSpout(ConnectionProvider connectionProvider) {
        this(connectionProvider,"pending");
    }
    
    public PersonaSpout(ConnectionProvider connectionProvider,String status) {
    ...
    public void nextTuple() {
        ...
        String sql = "select userKey,hierarchyId,phaseId,personaId from user_persona where status=? limit 50";
        logger.debug("try to query candidate user-personas.[SQL]"+sql+"[query]"+queryParams);
        ...
        if (result != null && result.size() != 0) {
            for (List<Column> row : result) {
        		...
        		sql = "update user_persona set status='pending',revision=revision+1";
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declare(new Fields("userKey","hierarchyId","phaseId","personaId"));
    }
