...
public class EvaluateMeasureSpout extends BaseRichSpout implements IRichSpout {
    ...
    public void nextTuple() {
        ...
        String sql = "select itemKey,evaluate,type,script,category,itemKey as itemKey2,evaluate as evaluate2 from evaluate where status='pending' and priority>=900 order by priority desc limit 50";
        ...
        if (result != null && result.size() != 0) {//如果有则直接发射
            for (List<Column> row : result) {
        		...
        		sql = "update evaluate set status='pending',revision=revision+1 where priority>=900";
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declare(new Fields("itemKey","evaluate","type","script","category","itemKey2","evaluate2"));//便于后续查询，作为冗余参数传递
    }
