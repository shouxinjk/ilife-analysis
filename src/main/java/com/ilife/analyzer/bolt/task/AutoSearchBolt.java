...
package com.ilife.analyzer.bolt.task;
...
/**
 * 
 * 执行自动搜索，根据用户或画像query、tags自行搜索
 * 
 * TODO
 * 1，根据query或tags发起ES搜索
 * 2，将搜索结果写入候选列表
 * 
 */
public class AutoSearchBolt extends BaseRichBolt {
    private static final Logger logger = LoggerFactory.getLogger(AutoSearchBolt.class);
    ...
    public AutoSearchBolt(ConnectionProvider connectionProvider) {
		this(connectionProvider,connectionProvider);
    }
    
    public AutoSearchBolt(ConnectionProvider connectionProviderBiz,ConnectionProvider connectionProviderAnalyze) {
    ...
    public void prepare(Map map, TopologyContext topologyContext, OutputCollector collector) {
        ...
        typePriority.put("constraint", 600);
