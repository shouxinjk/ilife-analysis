 ...
 * 
 * 根据userKey、Category建立measure-property任务记录，后续进行叶子节点维度评估
 * 1，根据category从业务库内查询dimension-measure记录
 * 2，将userKey、category、dimension、measure、weight属性关系写入分析库
 * 3，根据category从业务库内查询dimension-dimension记录
 * 4，将userKey、category、dimension、dimension、weight属性关系写入分析库
 * 
 */
public class CreateMeasureTaskBolt extends BaseRichBolt {
    ...
    String[] inputFields = {"_key"};//输入字段包含userKey
    ...
    public void execute(Tuple tuple) {
        ...
        //queryParams.add(new Column("category", category, Types.VARCHAR));
        	...
        	//select dim.parent_id as parent,dim.id as dimension, dim.weight as weight,dim.parent_ids as depth from mod_category cat,mod_item_dimension dim where cat.name=? and cat.id=dim.category
        		//2，写入分析库
        	//insert ignore into measure (userKey,parent,dimension,weight,status,priority,createdOn,modifiedOn) values()
        		String sqlQuery = "select dim.parent_id as parent,dim.id as dimension, dim.weight as weight,dim.parent_ids as depth from mod_user_dimension dim";
            logger.info("try to query pending measure-dimension.[SQL]"+sqlQuery+"[param]"+queryParams);
            String sqlInsert = "insert ignore into user_measure (userKey,parent,dimension,weight,status,priority,revision,createdOn,modifiedOn) values(?,?,?,?,'pending',?,1,now(),now())";
    	...
    	//select int.dimension_id as dimension,int.measure_id as measure, int.weight as weight from mod_category cat,int_item_dimension_measure int where cat.name=? and cat.id=int.category
    		//2，写入分析库
    	//insert ignore into measureproperty (userKey,dimension,property,weight,status,priority,createdOn,modifiedOn) values()
        sqlQuery = "select inter.dimension_id as dimension,inter.measure_id as measure, inter.weight as weight from int_user_dimension_measure inter";
        logger.debug("try to query pending measure-property.[SQL]"+sqlQuery+"[param]"+queryParams);
        sqlInsert = "insert ignore into user_measure_property (userKey,dimension,property,weight,status,priority,revision,createdOn,modifiedOn) values(?,?,?,?,'pending',?,1,now(),now())";
        ...
        if (result != null && result.size() != 0) {
            for (List<Column> row : result) {
            		...
            		items.add(item);
    	    			//更新对应叶子measure记录的优先级
            		String updatePrioritySql = "update user_measure set priority='900' where userKey='_USERKEY' and dimension='_DIMENSION'"
